...
**Nota DevSecOps (imágenes y cadena de suministro):**
En entornos de producción es una buena práctica evitar el uso de la etiqueta `latest` en las imágenes de Docker y utilizar en su lugar etiquetas versionadas e inmutables (por ejemplo, `v1.2.3`). Esto facilita la trazabilidad, la reproducibilidad de los despliegues y la respuesta ante incidentes. Además, los contenedores que ejecutas con Docker y orquestas con Kubernetes pueden integrarse con herramientas de escaneo de imágenes, generación de SBOM (Software Bill of Materials) y firma de imágenes, lo que fortalece la seguridad de la cadena de suministro de software.
...
Minikube es una herramienta que te permite configurar un clúster de Kubernetes local en tu máquina de desarrollo. Es un recurso valioso para ingenieros de datos o desarrolladores que desean experimentar, desarrollar y probar flujos de trabajo de procesamiento de datos en un entorno de Kubernetes.
...
Desde una perspectiva DevSecOps, las mismas imágenes de contenedor que ejecutas con Docker y despliegas en Kubernetes pueden pasar por pipelines de seguridad que incluyen escaneo de vulnerabilidades, generación de SBOM y firma de imágenes antes de ser aceptadas en entornos de producción. Esto ayuda a controlar mejor los riesgos de la cadena de suministro de software y a mantener clústeres más seguros.
